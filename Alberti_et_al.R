...
modelo0 <- glmmTMB(
  riq ~ yr * Herbivory + (1 | Plot) + ar1(yr + 0 | uidplot),
  family = gaussian("log"),
  data = riqueza,
  REML = T
)
...
check_model(modelo0) # Visual check of assumptions
Anova(modelo0)       # Type II Wald chi-square tests for fixed effects
emmeans(modelo1, pairwise ~ Herbivory | yr) # Pairwise comparisons by year
...
modelo0 <- glmmTMB(
  cov ~ yr * Herbivory + ar1(yr + 0 | uidplot)+ (1 | Plot), 
  data = riqueza,
  REML = T
)
...
modelo0 <- glmmTMB(
  log10(Litter) ~ yr * Herbivory + ar1(yr + 0 | uidplot) + (1 | Plot),
  data = litter,
  REML = T)
modelo1 <- update(modelo0, . ~ . - ar1(yr + 0 | uidplot))
anova(modelo0, modelo1)
...
check_model(modelo0)
Anova(modelo0)
emmeans(modelo0, pairwise ~ Herbivory|yr)
...
modelo0 <- glmmTMB(
  dist ~ yr * Herbivory + ar1(yr + 0 | uidplot)+(1 | Plot),
  family = gaussian("log"),
  data = beta,
  REML = T)
modelo1 <- update(modelo0, . ~ . - ar1(yr + 0 | uidplot))
...
check_model(modelo0)
Anova(modelo0)
emmeans(modelo0, pairwise ~ Herbivory)
...
modelo2 <- update(modelo1, dispformula = ~1)
anova(modelo2, modelo1)

check_model(modelo1)
Anova(modelo1)
emmeans(modelo1, pairwise ~ Herbivory)
...
# Modeling dispersion (variance) as a function of Herbivory to handle heteroscedasticity
mod <- glmmTMB(
  po4 ~ Herbivory + (1 | Plot),
  dispformula = ~Herbivory,
  nut_ex,
  REML = T
)
...
anova(mod, mod0) # Testing significance of the dispersion formula

mod1 <- update(mod, . ~ . - (1 | Plot))
anova(mod, mod1)
...
ggplot(
  nut_ex |>
    mutate(Herbivory = fct_relevel(Herbivory, "Control", "Partial")),
  aes(Herbivory, po4, color = Herbivory)
) +
  stat_summary() +
  geom_text(
    data = tukey_fo |>
  ...
  labs(x = "", y = "Phosphate concentration", color = "") +
  theme(
    legend.position = "top",
    strip.background = element_rect(fill = "white", color = "black"),
    legend.justification = "center",
...
# 8.2 Total Inorganic Nitrogen (TIN = NH4 + NO3) ####

mod <- glmmTMB(
  nh4 + no3 ~ Herbivory + (1 | Plot),
  dispformula = ~Herbivory,
  nut_ex,
  REML = T
)
...
modelo0 <- glmmTMB(
  organic_matter ~ Herbivory + (1 | Plot),
  data = nut_ex,
  REML = T
)
...
# 8.4 CO2 emissions (Soil respiration) ####

datos_licor_seq <- read.csv("datos_licor_seq.csv") |> 
  mutate(Plot = as.character(Plot))

glmmTMB(
  pendiente ~ Herbivory + (1 | Plot),
  datos_licor_seq,
  dispformula = ~Herbivory,
  REML = T
) -> mod
...
(ggplot(
  datos_licor_seq |>
    ungroup() |>
    mutate(Herbivory = fct_relevel(Herbivory, "Control", "Partial")),
  aes(Herbivory, pendiente, color = Herbivory)
) +
    stat_summary() +
    geom_text(
      data = tukey_re |>
    ...
    labs(x = "", y = "Soil respiration", color = "") +
    theme(
      legend.position = "top",
      strip.background = element_rect(fill = "white", color = "black"),
      legend.justification = "center",
...
plot_grid(
  ...
  plot_grid(
    fo + theme(legend.position = "none"),
    ni + theme(legend.position = "none"),
    om + theme(legend.position = "none"),
    re + theme(legend.position = "none"),
    labels = "AUTO",
    ncol = 2,
    align = "vh", axis = "tblr",
...
mod01 <- glmmTMB(
  droppings ~ Herbivory * yr + (1 | Plot) + ar1(yr + 0 | uidplot),
  REML = T,
  hares,
  family = poisson()
)
mod1 <- update(mod01, . ~ . - ar1(fecha + 0 | uidplot))
anova(mod01, mod1)
mod2 <- update(mod01, . ~ . - (1 | Plot))
anova(mod2, mod01)
